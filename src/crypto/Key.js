...
const signOptions = {
//  recovered: false, // always true now
  lowS: true,
};
...
export default class Key {
  ...
  isValid() {
    this.calcValue();
    return 0n < this.value && this.value < secp256k1.CURVE.n;
  }
  ...
  fromString(str) {
    ...
    if (!arrayBuffersAreEqual(check, hashCheck)) {
  ...
  publicKey() {
    ...
    const point = secp256k1.ProjectivePoint.BASE.multiply(this.value);
  ...
  async sign(msgHash) {
    ...
    this.calcBytes();
    const signature = secp256k1.sign(msgHash.toBytes(), this.bytes, signOptions);
    return new Signature(signature.toDERRawBytes());
  }
...
export function addPrivateKeys(key1, key2) {
  ...
  const m = mod(sum, secp256k1.CURVE.n);
  return new Key(m);
}
