...
export default class XKey {
  ...
  static async fromSeed(value) {
    ...
    const key = new Uint8Array(hexToArrayBuffer('426974636f696e2073656564'));
  ...
  fromString(str) {
    ...
    if (!arrayBuffersAreEqual(check, hashCheck)) {
      throw new Error(
        `checksum invalid : check ${arrayBufferToHex(
          check,
        )}, hash ${arrayBufferToHex(hashCheck)}`,
      );
    }
  ...
  async deriveChild(index) {
    ...
    if (index >= Bip32Hardened) {
      ...
      buf.write(this.keyBytes);
    } else {
      // Write compressed public key
      if (this.isPrivate()) {
        buf.write(this.publicKey().toBytes());
      } else {
        buf.write(this.keyBytes);
      }
    }
