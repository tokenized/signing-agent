...
export default class PublicKey {
  constructor(value) {
    if (value) {
      if (value instanceof secp256k1.ProjectivePoint) {
        this.value = value;
  ...
  fromString(str) {
    const bytes = new Uint8Array(hexToArrayBuffer(str));
    this.fromBytes(bytes);
  }

  toString() {
    this.calcBytes();
    return arrayBufferToHex(this.bytes);
  }
  ...
  calcValue() {
    if (this.value && this.value instanceof secp256k1.ProjectivePoint) {
      return;
    }
    ...
    this.value = secp256k1.ProjectivePoint.fromCompressedHex(this.bytes);
  }
